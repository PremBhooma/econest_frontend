...
function Assignflattocustomer({ closeFlatToCustomer, flatNo, block_id, refreshUserDetails }) {
    ...
    useEffect(() => {
        if (flatNo) getFlatsData(flatNo, block_id)
    }, [flatNo, block_id])

    async function getFlatsData(flat, block_id) {
        try {
            setFlatLoading(true);
            ...
            const fetchedFlats = data?.data || [];
            setFlat(fetchedFlats);

            if (flatNo && fetchedFlats.length > 0) {
                const matchedFlat = fetchedFlats[0];
                setSelectedFlat(matchedFlat);
                setSearchedFlat(matchedFlat?.label || "");
                setSaleableAreaSqFt(matchedFlat?.square_feet)
            }
            return true;
        } catch (error) {
            console.log(error);
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-end bg-black/50 backdrop-blur-[2px]" onClick={closeFlatToCustomer}>
            <div
                className="w-full h-[90vh] sm:h-full bg-white overflow-hidden flex flex-col animate-in slide-in-from-right duration-300"
...
export default Assignflattocustomer;
