...
const Bankstab = () => {
    ...
    async function getBanksData(newPage = page, currentLimit = limit) {
        setIsLoading(true);
        try {
            const response = await Settingsapi.get(`get-all-banks-list?page=${newPage}&limit=${currentLimit}&searchQuery=${searchQuery}`);
    ...
    const confirmDeleteBank = async () => {
        ...
        setIsLoading(true);
        try {
            const response = await Settingsapi.post("delete-bank", { id: bankIdToDelete });
